      ...
      <div ref={containerRef} className="flex flex-col gap-2 p-3">
        ...
        {[...items, ...items, ...items].map((item, i) => {
          ...
          return (
            <div
              key={`${item.name}-${i}`}
              className="xl:h-[8vh] h-[6vh] flex items-center justify-between bg-white xl:text-[1.3vw] text-[4vw] font-semibold rounded-[55px] md:px-4 py-4 px-4 transition-colors duration-300 hover:bg-[#F7F6F6] cursor-pointer"
            >
              {item.name}
